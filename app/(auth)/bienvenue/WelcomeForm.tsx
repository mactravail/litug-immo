...
const INPUT =
  'w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm text-text placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent';

export function WelcomeForm() {
  const [state, action, isPending] = useActionState(setInitialPassword, null);
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        <input id="confirm" name="confirm" type="password" required minLength={8} placeholder="••••••••" autoComplete="new-password" className={INPUT} />
      </div>
      ...
      >
        {isPending ? 'Enregistrement…' : 'Accéder à mon espace'}
      </button>
    </form>
  );
}
